...
exports.readSequenceNumber = function (format, buffer) {
    ...
    else if (format == 6) return buffer.readUInt8BE(17);
...
exports.isPressureSupported = function (format, buffer) {
    if (format == 5 && buffer.readUInt16BE(7) == 65535) return false;
    else return true;
}

exports.isHumiditySupported = function (format, buffer) {
    if (format == 5 && buffer.readUInt16BE(5) == 65535) return false;
    else return true;
}

exports.readCo2 = function (format, buffer) {
    if (format == 6) return buffer.readUInt16BE(11);
    else if (format == 225) return buffer.readUInt16BE(17);
    else throw new Error(`Unsupported format detected`);
}

exports.readPm25 = function (format, buffer) {
    if (format == 6) return buffer.readUInt16BE(9) * 0.1;
    else if (format == 225) return buffer.readUInt16BE(11) * 0.1;
    else throw new Error(`Unsupported format detected`);
}

exports.readPm1 = function (format, buffer) {
    if (format == 225) return buffer.readUInt16BE(9) * 0.1;
    else throw new Error(`Unsupported format detected`);
}

exports.readPm4 = function (format, buffer) {
    if (format == 225) return buffer.readUInt16BE(13) * 0.1;
    else throw new Error(`Unsupported format detected`);
}

exports.readPm10 = function (format, buffer) {
    if (format == 225) return buffer.readUInt16BE(15) * 0.1;
    else throw new Error(`Unsupported format detected`);
}

exports.readNoxIndex = function (format, buffer) {
    if (format == 6) return buffer.readUInt8(14) * 2 + ((buffer.readUInt8(18) >> 7) & 0x01);
    else if (format == 225) return buffer.readUInt8(18);
    else throw new Error(`Unsupported format detected`);
}

exports.readTvocIndex = function (format, buffer) {
    if (format == 6) return buffer.readUInt8(13) * 2 + ((buffer.readUInt8(18) >> 6) & 0x01);
    else if (format == 225) return buffer.readUInt8(17);
    else throw new Error(`Unsupported format detected`);
}

exports.readLuminance = function (format, buffer) {
    if (format == 6) {
        value = buffer.readUInt8(15);
        const MAX_VALUE = 65535;
        const MAX_CODE = 254;
        const DELTA = Math.log(MAX_VALUE + 1) / MAX_CODE;
        const CODE = Math.round(Math.log(value + 1) / DELTA);
        return Math.exp(CODE * DELTA) - 1;
    }
    else if (format == 225) return buffer.readUIntBE(21, 3) * 0.01;
    else throw new Error(`Unsupported format detected`);
}
...
exports.calc_aqi = function (pm25, co2) {
